...
def construct_knn_approx(train_inputs, train_targets, k):
    ...
    def prediction_function(inputs):
        ...
        for i in range(train_inputs.shape[1]):
            ...
            train_input = np.resize(train_input, (1, train_input.size))
            input = inputs[:,i]
            input = np.reshape(input,(-1,1))
            #input = input.reshape((input.size,1))
            distances = distance(train_input,input)
            dis += distances
        #print("dis",dis.shape)
        #predicts = np.empty(inputs.size)
        predicts = np.empty(dis.shape[0])
        for i, neighbourhood in enumerate(np.argpartition(dis, k)[:,:k]):
...
def prediction_function(xs, weights, degree):
